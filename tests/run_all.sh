...
echo ""; echo "${BOLD}=========================================${RESET}"; echo "${BOLD}  systemd-lab — Automated Test Suite${RESET}"; echo "${BOLD}=========================================${RESET}"; echo ""
log_info "Workspace: $WORKSPACE_DIR"; log_info "Server port: $SERVER_PORT"
assert "VM is reachable via SSH" ssh_vm "echo ok"
ci=$(ssh_vm "cloud-init status 2>/dev/null || echo done") || true; assert_contains "Cloud-init is done" "$ci" "done|status: done"
cleanup_systemd

TOTAL_PASS=0; TOTAL_FAIL=0; TESTS_RUN=0; TESTS_SKIPPED=0; FAILED_EXERCISES=()
run_test() { local n="$1"; local f=($TESTS_DIR/test_${n}_*.sh); [[ ! -f "${f[0]}" ]] && return; should_skip "$n" && { log_info "Skipping $n"; TESTS_SKIPPED=$((TESTS_SKIPPED+1)); return; }; cleanup_systemd >/dev/null 2>&1; local e=0; bash "${f[0]}" || e=$?; TESTS_RUN=$((TESTS_RUN+1)); if [[ "$e" -ne 0 ]]; then TOTAL_FAIL=$((TOTAL_FAIL+1)); FAILED_EXERCISES+=("$n"); else TOTAL_PASS=$((TOTAL_PASS+1)); fi; }
