...
SERVER_PORT="$(_get_port systemd-lab)"
if [[ -z "$SERVER_PORT" ]]; then echo "ERROR: Cannot find VM port. Is systemd-lab running?"; exit 1; fi
...
cleanup_systemd() {
    ssh_vm "sudo systemctl disable --now custom.timer hello.service 2>/dev/null; sudo rm -f /etc/systemd/system/hello.service /etc/systemd/system/custom.service /etc/systemd/system/custom.timer /tmp/hello-systemd.log /tmp/custom-timer.log 2>/dev/null; sudo systemctl daemon-reload 2>/dev/null; sudo systemctl start sample.service 2>/dev/null; sudo systemctl enable sample.service 2>/dev/null" 2>/dev/null || true
}
